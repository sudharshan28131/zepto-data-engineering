...
select *
from clean_sch.restaurant_location_stm ;
select *
from clean_sch.restaurant_location
...
select *
from consumption_sch.restaurant_location_dim
order by location_id;
-- load the delta data
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from  @csv_stg/delta/location/delta-day02-2rows-update.csv t)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;


select *
from stage_sch.location;

select *
from stage_sch.location_stm;

CREATE OR REPLACE TEMP TABLE common.location_temp AS
SELECT *
FROM stage_sch.location_stm;
